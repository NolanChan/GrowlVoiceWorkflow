...
class FuzzyDict(dict):
    ...
    def _search(self, lookfor, stop_on_first = False):
        ...
        for key in self:
            ...
            if ratio > best_ratio:
                ...
                best_ratio = ratio 
                best_key = key
                best_match.append(self._dict_getitem(key))
    ...
    def __getitem__(self, lookfor):
        ...
        if not matched:
            return "Sorry, the closest match to '%s' is '%s'"%(str(lookfor), str(key))
            # for the Alfred workflow, I don't want to be raising an error. Just return the closest match
            # raise KeyError(
